...
def formRoleName():
    ...
    if not stageFileFound:
        raise Exception(
            "Stage file is not found in the current working directory. Please add the stage file into the specified directory"
        )
    else:
        print(
            "Found the stage file "
            + fileName
            + extension
            + " in the current working directory"
        )
        with open(os.path.join(sys.path[0], fileName + extension), "r") as file:
            ...
            if configuration is None:
                raise Exception(
                    "Stage file is empty. Please use the stage file content that you have used during SWB installation"
                )
            print("Checking for the key solutionName in the stage file")
            if "solutionName" not in configuration.keys():
                raise Exception(
                    "Solution name does not exist in your stage file. Please add solution name into your stage file"
                )
            if configuration["solutionName"] is None:
                raise Exception(
                    "Solution name value does not exist in your stage file. Please add solution name that you have used during SWB installation into your stage file"
                )
            ...
            if "awsRegion" not in configuration.keys():
                raise Exception(
                    "AWS region does not exist in your stage file. Please add aws region into your stage file"
                )
            if configuration["awsRegion"] is None:
                raise Exception(
                    "AWS Region value does not exist in your stage file. Please add region name that you have used during SWB installation into your stage file"
                )
            # Setting boto3 to use provided awsProfile and awsRegion from stage file
            boto3.setup_default_session(
                profile_name=configuration["awsProfile"],
                region_name=configuration["awsRegion"],
            )
            ...
            stackName = commonName + "-rlrstudio-penta"
...
def getPortfolioIdUsingRoleName(roleName):
    ...
    if roleName.endswith("-LaunchConstraint"):
        ...
        response = client.list_portfolios(AcceptLanguage="en", PageSize=10)
        ...
        if portfolioFound:
            ...
            raise Exception(
                "Portfolio with the name "
                + portfolioName
                + " is not found in the account. Please check the parameters in the stage file"
            )
    return portfolioId
...
def checkIfRegionisValid(region):
    print(
        "Checking for the validity of region " + region + " which is in the stage file"
    )
    client = boto3.client("ec2")
    response = client.describe_regions(RegionNames=[region])
...
def checkIfPortfolioExistInAccount():
    print(
        "Checking for the Portfolio ID "
        + configuration["portfolioId"]
        + " in the account "
    )
    client = boto3.client("servicecatalog")
    response = client.describe_portfolio(
        AcceptLanguage="en", Id=configuration["portfolioId"]
    )
...
def createStack(roleName, stackName, bucketUrl):
    ...
    if roleExist:
        ...
        if portfolioId == "":
            raise Exception(
                "Portfolio ID value is not available. Please enter Portfolio ID value into stage file or check your SWB installated account"
            )
        ...
        response = client.create_stack(
            ...
            Parameters=[
                {"ParameterKey": "PortfolioID", "ParameterValue": portfolioId},
                {"ParameterKey": "RoleName", "ParameterValue": roleName},
                {"ParameterKey": "TemplateUrl", "ParameterValue": bucketUrl},
            ],
        )
    else:
        raise Exception(
            "Launch Role "
            + roleName
            + " does not exist in your main SWB installation account. Please contact your administrator"
        )
    return response
...
def getStackEvents(stackName):
    ...
    global eventsList
    print(
        "-------------------------Started creating the stack----------------------------------"
    )
    print(
        "Status                   " + "Logical ID                  " + "Status Reason"
    )
    print(
        "-------------------------------------------------------------------------------------"
    )
    for i in range(3):
        ...
        finalList = []
        time.sleep(10)
        stackEventsResponse = client.describe_stack_events(StackName=stackName)
        ...
        for k, v in mainList.items():
            ...
            print(
                "{:<24} {:<28} {:<30}".format(status, LogicalResourceId, statusReason)
            )
...
def getArtifactsBucketName(commonName):
    ...
    response = s3Client.head_bucket(Bucket=artifactsBucketName)
    if artifactsBucketName == "":
        raise Exception(
            "SWB artifacts bucket name is not found. Please check SWB installation or the stage file"
        )
...
def uploadRstudioTemplateToArtifactsBucketAndgetTheURL(bucketName):
    ...
    with open(templatePath, "rb") as f:
        response = s3Client.upload_file(
            templatePath, bucketName, "partner-products/{}".format(objectName)
        )
        preSignedUrlResponse = s3Client.generate_presigned_url(
            "get_object",
            Params={
                "Bucket": bucketName,
    ...
    if bucketUrl == "":
        raise Exception(
            "Failed in uploading the RStudio template to the bucket. Please make sure all the parameters in stage file are correct."
        )
    else:
        print(
            "Completed uploading the EC2-Rstudio server template to SWB artifacts bucket"
        )
    return bucketUrl
...
if __name__ == "__main__":
    try:
        ...
        if (
            "ResourceNotFoundException" in e.message
            and "DescribePortfolio" in e.message
        ):
            ...
            print(
                "Launch Role does not exist in your main SWB installation account. Please contact your administrator or check your stage file"
            )
        elif "InvalidParameterValue" in e.message and "DescribeRegions" in e.message:
            print(
                "AWS region name provided in the stage file is invalid. Please add region name that you have used during SWB installation into your stage file"
            )
        elif "AlreadyExistsException" in e.message and "CreateStack" in e.message:
            print(
                "A stack with the same configurations already exist. So the RStudio-server template will be available as part of your current SWB deployment. Please check by logging in as an administrator into SWB"
            )
        elif "Not Found" in e.message:
            print(
                "Please use the AWS credentials of SWB deployment in the aws Configure or check the stage file"
            )
        else:
            print("Exception ", e)
        if stackName != "":
